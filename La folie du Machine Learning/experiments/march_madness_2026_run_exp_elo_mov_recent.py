...
def compute_elo_ratings(compact_results, k=20, home_adv=100, mean_revert=0.75,
    ...
    for _, row in sorted_games.iterrows():
        ...
        # Mise à jour (optionnel: pondération par marge de victoire)
        k_eff = k
        if use_mov:
            margin = abs(float(row.get('WScore', 0)) - float(row.get('LScore', 0)))
            margin = min(margin, mov_cap)
            # Pondération standard-ish Elo MoV (stabilisée pour éviter les extrêmes)
...
def compute_recent_team_stats(detail_df, n_recent=12):
    """
    Calcule des stats agrégées de forme récente (n derniers matchs de saison régulière)
    par équipe/saison, avec des colonnes préfixées 'Recent_'.
    """
    records = []
    ...
    for prefix, team_col, opp_prefix in [('W', 'WTeamID', 'L'), ('L', 'LTeamID', 'W')]:
        ...
        sub = df[['Season', 'DayNum', team_col, 'eFGpct', 'TOVpct', 'ORBpct', 'FTr',
                  'Opp_eFGpct', 'Opp_TOVpct', 'Opp_ORBpct', 'Opp_FTr',
                  f'{prefix}Score', f'{opp_prefix}Score']].copy()
        sub.columns = ['Season', 'DayNum', 'TeamID', 'eFGpct', 'TOVpct', 'ORBpct', 'FTr',
                       'Opp_eFGpct', 'Opp_TOVpct', 'Opp_ORBpct', 'Opp_FTr',
                       'Score', 'OppScore']
        sub['Won'] = 1 if prefix == 'W' else 0
        records.append(sub)
    ...
    recent_games = (
        all_games.groupby(['Season', 'TeamID'], group_keys=False)
        .tail(n_recent)
        .copy()
    )

    recent = recent_games.groupby(['Season', 'TeamID']).agg(
        Recent_WinPct=('Won', 'mean'),
        Recent_AvgMargin=('Margin', 'mean'),
        Recent_eFGpct=('eFGpct', 'mean'),
        Recent_TOVpct=('TOVpct', 'mean'),
        Recent_ORBpct=('ORBpct', 'mean'),
        Recent_FTr=('FTr', 'mean'),
        Recent_Opp_eFGpct=('Opp_eFGpct', 'mean'),
        Recent_Opp_TOVpct=('Opp_TOVpct', 'mean'),
        Recent_Opp_ORBpct=('Opp_ORBpct', 'mean'),
        Recent_Opp_FTr=('Opp_FTr', 'mean'),
        Recent_GamesPlayed=('Won', 'count'),
    ).reset_index()

    recent['Recent_Net_eFG'] = recent['Recent_eFGpct'] - recent['Recent_Opp_eFGpct']
    recent['Recent_Net_TOV'] = recent['Recent_Opp_TOVpct'] - recent['Recent_TOVpct']
    recent['Recent_Net_ORB'] = recent['Recent_ORBpct'] - recent['Recent_Opp_ORBpct']
    recent['Recent_Net_FTr'] = recent['Recent_FTr'] - recent['Recent_Opp_FTr']

    # Garder uniquement les colonnes utiles pour le modèle
    keep_cols = [
        'Season', 'TeamID',
        'Recent_WinPct', 'Recent_AvgMargin',
        'Recent_eFGpct', 'Recent_TOVpct', 'Recent_ORBpct', 'Recent_FTr',
        'Recent_Net_eFG', 'Recent_Net_TOV', 'Recent_Net_ORB', 'Recent_Net_FTr',
        'Recent_GamesPlayed',
    ]
    return recent[keep_cols]

print('Calcul stats hommes...')
m_stats = compute_team_stats(m_reg_detail)
print(f'  -> {len(m_stats):,} paires (équipe, saison)')
print(f'Calcul stats récentes hommes (derniers {RECENT_N_GAMES} matchs)...')
m_stats_recent = compute_recent_team_stats(m_reg_detail, n_recent=RECENT_N_GAMES)
m_stats = m_stats.merge(m_stats_recent, on=['Season', 'TeamID'], how='left')
print(f'  -> stats fusionnées hommes : {len(m_stats):,} paires (équipe, saison)')

print('Calcul stats femmes...')
w_stats = compute_team_stats(w_reg_detail)
print(f'  -> {len(w_stats):,} paires (équipe, saison)')
print(f'Calcul stats récentes femmes (derniers {RECENT_N_GAMES} matchs)...')
w_stats_recent = compute_recent_team_stats(w_reg_detail, n_recent=RECENT_N_GAMES)
w_stats = w_stats.merge(w_stats_recent, on=['Season', 'TeamID'], how='left')
print(f'  -> stats fusionnées femmes : {len(w_stats):,} paires (équipe, saison)')
...
def build_training_pairs(tourney_df, seeds_df, elo_dict, stats_df, massey_df=None, gender='M'):
    ...
    for _, game in tourney_df.iterrows():
        ...
        # Stats
        stat_cols = [
            'WinPct', 'AvgMargin', 'eFGpct', 'TOVpct', 'ORBpct', 'FTr',
...
DELTA_STAT_FEATURES = [
    ...
    'Delta_Recent_WinPct', 'Delta_Recent_AvgMargin',
    'Delta_Recent_eFGpct', 'Delta_Recent_TOVpct', 'Delta_Recent_ORBpct', 'Delta_Recent_FTr',
    'Delta_Recent_Net_eFG', 'Delta_Recent_Net_TOV', 'Delta_Recent_Net_ORB', 'Delta_Recent_Net_FTr',
    'Delta_Recent_GamesPlayed',
]
