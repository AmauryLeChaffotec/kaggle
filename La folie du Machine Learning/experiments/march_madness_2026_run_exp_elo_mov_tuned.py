...
EXP_TAG = 'exp_elo_mov_tuned'
ELO_TUNE_TRIALS_M = 12
ELO_TUNE_TRIALS_W = 16
ELO_TUNE_RECENCY_WEIGHT = 0.03  # pondère légèrement les saisons récentes
ELO_TUNE_RANDOM_SEED = 42
...
def build_elo_eval_pairs(tourney_df, recency_weight=0.03):
    """Paires orientées (TeamA=min TeamID) pour scorer un Elo de fin de saison sur le tournoi."""
    records = []
    for _, game in tourney_df.iterrows():
        ...
        records.append({'Season': season, 'TeamA': team_a, 'TeamB': team_b, 'Target': target})
    eval_df = pd.DataFrame(records)
    min_season = eval_df['Season'].min()
    eval_df['SampleWeight'] = 1.0 + recency_weight * (eval_df['Season'] - min_season)
    return eval_df

def elo_tourney_brier(elo_dict, eval_df):
    """Brier score d'un Elo pur sur les matchs de tournoi (proba via DeltaElo)."""
    from sklearn.metrics import brier_score_loss

    preds = []
    for _, row in eval_df.iterrows():
        elo_a = elo_dict.get((int(row['Season']), int(row['TeamA'])), 1500)
        elo_b = elo_dict.get((int(row['Season']), int(row['TeamB'])), 1500)
        p_a = 1.0 / (1.0 + 10.0 ** ((elo_b - elo_a) / 400.0))
        preds.append(p_a)
    preds = np.array(preds, dtype=float)
    score = brier_score_loss(eval_df['Target'].values, preds, sample_weight=eval_df['SampleWeight'].values)
    return score

def tune_elo_mov_params(compact_results, tourney_df, default_params, n_trials=12, gender_label='M'):
    """
    Tuning ciblé des paramètres Elo MoV avec Optuna.
    Objectif: Brier d'un Elo pur sur le tournoi (pondéré vers les saisons récentes).
    """
    import optuna

    optuna.logging.set_verbosity(optuna.logging.WARNING)
    eval_df = build_elo_eval_pairs(tourney_df, recency_weight=ELO_TUNE_RECENCY_WEIGHT)
    print(f'Tuning Elo MoV {gender_label} ({n_trials} essais) sur {len(eval_df):,} matchs de tournoi...')

    # Bornes légèrement différentes par genre (home_adv souvent plus faible en W)
    if gender_label == 'M':
        home_adv_low, home_adv_high = 70, 130
    else:
        home_adv_low, home_adv_high = 40, 110

    baseline_elo = compute_elo_ratings(compact_results, use_mov=True, **default_params)
    baseline_score = elo_tourney_brier(baseline_elo, eval_df)
    print(f'  Brier Elo baseline ({gender_label}) : {baseline_score:.4f}')

    def objective(trial):
        params = {
            'k': trial.suggest_float('k', 12.0, 36.0),
            'home_adv': trial.suggest_int('home_adv', home_adv_low, home_adv_high),
            'mean_revert': trial.suggest_float('mean_revert', 0.65, 0.90),
            'mov_cap': trial.suggest_int('mov_cap', 12, 40),
            'mov_scale': trial.suggest_float('mov_scale', 1.2, 4.5),
        }
        elo_dict = compute_elo_ratings(compact_results, use_mov=True, **params)
        return elo_tourney_brier(elo_dict, eval_df)

    sampler = optuna.samplers.TPESampler(seed=ELO_TUNE_RANDOM_SEED)
    study = optuna.create_study(direction='minimize', sampler=sampler)
    study.enqueue_trial({
        'k': float(default_params['k']),
        'home_adv': int(default_params['home_adv']),
        'mean_revert': float(default_params['mean_revert']),
        'mov_cap': int(default_params['mov_cap']),
        'mov_scale': float(default_params['mov_scale']),
    })
    study.optimize(objective, n_trials=n_trials, show_progress_bar=True)

    best_params = {**study.best_params, 'use_mov': True}
    print(f'  Meilleur Brier Elo ({gender_label}) : {study.best_value:.4f}')
    print(f'  Params Elo ({gender_label}) : {best_params}')
    return best_params

m_elo_default_params = {'k': 22, 'home_adv': 95, 'mean_revert': 0.75, 'mov_cap': 25, 'mov_scale': 2.2}
w_elo_default_params = {'k': 20, 'home_adv': 80, 'mean_revert': 0.78, 'mov_cap': 25, 'mov_scale': 2.2}

m_elo_params = tune_elo_mov_params(
    m_reg_compact, m_tourney, m_elo_default_params, n_trials=ELO_TUNE_TRIALS_M, gender_label='M'
)
w_elo_params = tune_elo_mov_params(
    w_reg_compact, w_tourney, w_elo_default_params, n_trials=ELO_TUNE_TRIALS_W, gender_label='W'
)

print('Calcul Elo hommes (MoV tuned)...')
m_elo = compute_elo_ratings(m_reg_compact, **m_elo_params)
print(f'  -> {len(m_elo):,} paires (saison, équipe)')

print('Calcul Elo femmes (MoV tuned)...')
w_elo = compute_elo_ratings(w_reg_compact, **w_elo_params)
